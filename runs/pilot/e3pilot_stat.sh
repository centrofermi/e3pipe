...
SET @PILOT_START_DATE := '2014-10-21';
SET @PILOT_END_DATE := '2014-11-14';

SELECT * from processing_status_codes;

SELECT processing_status_code, COUNT(*) from runs GROUP BY processing_status_code;

SELECT station_name, COUNT(*) from runs WHERE run_date >= @PILOT_START_DATE AND run_date <= @PILOT_END_DATE GROUP BY station_name;

SELECT COUNT(*) from runs WHERE run_date >= @PILOT_START_DATE AND run_date <= @PILOT_END_DATE;

SELECT SUM(num_events) from runs WHERE run_date >= @PILOT_START_DATE AND run_date <= @PILOT_END_DATE;

SELECT SUM(num_track_events) from runs WHERE run_date >= @PILOT_START_DATE AND run_date <= @PILOT_END_DATE;
