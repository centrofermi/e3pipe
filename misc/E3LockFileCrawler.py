...
class E3LockFileCrawler(E3FileCrawlerBase):

    """ Overloaded crawler class for the raw data.
    """
    
    ROOT_FOLDER = E3PIPE_LOCK_BASE

    def __init__(self, stations, endDate = None, daysSpanned = 1):
        """ Constructor.
        """
    ...
    def crawlFolder(self, folderPath):
        """ Overloaded methos.
        """
        fileList = glob.glob(os.path.join(folderPath, '*.lock'))
        fileList.sort()
        return fileList
...
def test():
    """ Test program.
    """
    crawler = E3LockFileCrawler('BOLO-03')
    print crawler
