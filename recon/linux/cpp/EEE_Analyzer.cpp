...
int main(int argc, char** argv)
{
	...
	std::ofstream outputFileOUT((name + "out").c_str());
	std::ofstream outputFileTIM((name + "tim").c_str());
	std::ofstream outputFile2TT((name + "2tt").c_str());
	std::ofstream outputFileSUM((name + "sum").c_str());
...
void merge(int& number, int* iHitR, int* iHitL, int* iHitTR, int* iHitTL, float* xChamber, float* yChamber, float* tChamber)
{
	...
	while (k < number - 1)
	{
		...
		if (std::abs((double)(strip1 - strip2)) < 2)
		{
