...
c	   write(*,*)' run mumber = ',run_char
	   READ (run_char, '(I10)'), irun_number
c	   write(*,*)' run number',irun_number
c
...
c
	   ...
	   if(iwadflag.eq.1.and.igeoflag.eq.1)goto 2234
		...
		if(ntype.eq.6) then
		iwadflag=1
c		write(*,*)' Have found WAD block'
c       write (*,*)" WAD Block"
c  	   write (*,*) (iarray(k),k=1,6)
...
c	  endif
	  ...
	  tstdc=(1.000001-t_delay)*(tns1/icalib(1)+tns2/icalib(2))*0.5
	  tstdc = tstdc+t_delay
      ...
      usec=time_ev-itime_ev
	  iusec=ifix(usec*1000000)
	  isec=time_ev

	  ananosec=isec
	  ananosec=time_ev-ananosec
	  ananosec=ananosec*1.0D9
	  inanosec=ananosec	
	  
c	  write (*,*) tns
c      time_ev=day*86400.D0+sec+(tns)/1.0D9
c    
...
c     $  F10.5,F10.5,1X,F8.5,1X,F10.3,1X,F10.2)
     ...
     $  chi_old,tof_ev,path
	    ...
	    write(*,*)' number of hits    ',n_b,n_m,n_u
	    write(*,*)' number of clusters',n_bc,n_mc,n_uc
		do jj=1,n_b
		...
		write(*,*)ihit_b_l,ihit_b_r,ihit_m_l,ihit_m_r,ihit_t_l,ihit_t_r
	   do jj=1,192
	     if(t(jj).gt.0.) then
		 write(*,*)jj,t(jj)
		 endif
